...
exports.applyForJob = async (req, res) => {
    try {
        ...
        if (job.status !== 'open') {
            return res.status(400).json({ success: false, error: 'This job is no longer open for applications' });
        }
...
exports.getJobApplications = async (req, res) => {
    try {
        ...
        const applications = await Application.find({ jobId: req.params.jobId })
            .populate('applicantId', 'name email'); // Lấy thêm thông tin của ứng viên
...
exports.updateApplicationStatus = async (req, res) => {
    try {
        ...
        if (application.jobId.creator.toString() !== req.user.id) {
